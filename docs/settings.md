...
| ES_ENABLE_CLIENT_FILTERS | true or false | Presents a web session attribute that enables QnABot to filter and block questions based on the session attribute.
...
| KENDRA_INDEXER_CRAWL_MODE | HOST_ONLY \| SUBDOMAINS \| EVERYTHING | The crawler will either crawl links only on the same host, subdomains of the host or any link it finds
